...
import improcessor.basic as improc
...
theCfg  = detect.CfgSGM().builtForBlackMatHSV()

preProc = improc.basic( cv2.cvtColor, (cv2.COLOR_RGB2HSV,) )

#==[2] Calibrate.
#
bgModel = detect.bgGaussian.buildAndCalibrateFromConfigRGBD(theCfg, preProc, theStream, True)
#bgModel = detect.bgGaussian(theCfg, preProc)
...
while(True):
    ...
    hsv = preProc.apply(rgb)

    display.binary(bgS.bgIm, ratio=0.25, window_name="Detection")
    display.rgb(hsv, ratio=0.25, window_name="HSV")
